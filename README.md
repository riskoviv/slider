...
### Опции и методы значений состояния
Принимают значение типа `boolean`, по умолчанию все имеют значение `false`

Опция | Описание | API-метод, меняющий эту опцию
--- | --- | ---
`isVertical` | горизонтальный / вертикальный вид | `setVerticalState`
`isInterval` | одиночное значение (1 бегунок) / интервал (2 бегунка) | `setInterval`
`showTip` | показывать ли плавающую подсказку над бегунком (или бегунками, если их 2) | `setShowTip`
`showScale` | показывать ли под слайдером шкалу с допустимыми значениями с учётом шага | `setShowScale`
`showProgressBar` | показывать ли прогресс бар: при одиночном значении от начала слайдера до бегунка, при интервале от первого до второго бегунка | `setShowProgress`

### Опции и методы числовых значений
Принимают значение типа `number`

Опция | Значение по умолчанию | Описание | API-метод, меняющий эту опцию
--- | --- | --- | ---
`stepSize` | 10 | размер шага, на который могут перемещаться бегунки начиная с минимального значения | `setStepSize`
`minValue` | -100 | минимальное значение слайдера | `setMinValue`
`maxValue` | 100 | максимальное значение слайдера | `setMaxValue`
`value1` | -50 | первое значение, на него указывает первый бегунок | `setValue1`
`value2` | 50 | второе значение, на него указывает второй бегунок | `setValue2`
