...
Logger – это класс, используемый в режиме разработки классами Model, EventEmitter и функцией инициализации плагина для продвинутого вывода и сохранения сообщений об ошибках и предупреждений, которые могут возникнуть при инициализации плагина и во время его работы.
...
После инициализации Presenter'а на корневой элемент слайдера, созданный внутри View, добавляются публичные методы для динамической кастомизации слайдера, используя их можно изменить любую из [опций](#api) плагина. Также добавляются методы для подписывания / отписывания на события слайдера других HTML-элементов страницы или callback-функций.
...
- Плагин создаётся на объекте типа `JQuery<HTMLElement>` путём вызова метода `sliderPlugin()` на этом элементе.
  ...
  Создать плагин со [**стандартными** опциями](#api) можно, если вызвать метод инициализации плагина без аргументов:
  ```js
  $('#slider').sliderPlugin();
  ```
